...
const updateAthlete = async (athleteArray) => {
    for (const athlete of athleteArray) {
        try {
            const res = await Athlete.findOneAndUpdate({ _id: athlete._id }, { solved: 'true' }, {
                new: true,
                runValidators: true
            })
            if (!athlete) {
                return res.status(404).json({ msg: `No athlete with id : ${athlete._id}` })
            }
        } catch (error) {
            console.log(error);
        }
...
const updateCoaches = async (coachArray) => {
    for (const coach of coachArray) {
        try {
            const res = await Coach.findOneAndUpdate({ _id: coach._id }, { lessons: coach.lessons }, {
                new: true,
                runValidators: true
            })
            if (!coach) {
                return res.status(404).json({ msg: `No athlete with id : ${coach._id}` })
            }
...
const createTimeTable = async (req, res) => {
    ...
    */
    ...
    for (const lesson of lessonArray) {
        ...
        for (const coach of coachArray) {
            ...
            if (
                ...
                lesson.end_time.setTime(lesson.end_time.getTime() + lesson.duration * 60 * 1000);
                lesson.solved = true
                //update coach
                coach.remShift -= lesson.duration
    ...
    res.status(200).json({ lessons: lessonArray, unsolvedAthletes: unsolvedAthletes })
